"""
Group-immutability classifier for V19 rules.

For every rule in data/rules_classified_v19_materiality.tsv, call Gemini with
prompt_V19_immutability.md and obtain:
- group_immutability (int 1-5)
- immutability_reasoning (short string)

Outputs:
- data/immutability_mapping_v19.json     cache keyed by rule_uid
- data/rules_classified_v19_full.tsv     rule-level table with all three
  secondary classifiers (specificity, materiality, immutability) merged.
"""
...
IN_TSV = HERE / "data/rules_classified_v19_materiality.tsv"
PROMPT_FILE = HERE / "prompt/prompt_V19_immutability.md"
MAP_JSON = HERE / "data/immutability_mapping_v19.json"
OUT_TSV = HERE / "data/rules_classified_v19_full.tsv"
...
def classify_batch(client: OpenAI, system_prompt: str, batch: list[dict]) -> dict:
    user_msg = (
        "Rate group near-immutability for each input rule. Return a JSON list.\n"
    ...
    for item in results:
        i = item.get("i")
        m = item.get("group_immutability")
        reason = item.get("immutability_reasoning", "")
        try:
            m = int(m) if m is not None else None
        except Exception:
            m = None
        out[i] = {"group_immutability": m, "immutability_reasoning": reason}
    return out
...
def main():
    ...
    if todo_rows:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(futs), desc="Immutability"):
                out = fut.result()
                if "error" in out:
    ...
    rules["group_immutability"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("group_immutability")
    )
    rules["immutability_reasoning"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("immutability_reasoning", "")
    )

    rules.to_csv(OUT_TSV, sep="\t", index=False)
    print(f"\nWrote {len(rules):,} rules → {OUT_TSV}")

    print("\nGroup immutability distribution:")
    print(
        rules["group_immutability"].value_counts(dropna=False).sort_index().to_string()
    )
