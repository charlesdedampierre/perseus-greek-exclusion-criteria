"""
Resource-materiality classifier for V19 rules.

For every rule in data/rules_classified_v19.tsv, call Gemini with
prompt_V19_materiality.md and obtain:
- resource_materiality (int 1-5)
- materiality_reasoning (short string)

Outputs:
- data/materiality_mapping_v19.json   cache keyed by rule_uid
- data/rules_classified_v19_materiality.tsv  rule-level table joined with the
  existing secondary-classified table.
"""
...
SECONDARY_TSV = HERE / "data/rules_classified_v19_secondary.tsv"
PROMPT_FILE = HERE / "prompt/prompt_V19_materiality.md"
MAP_JSON = HERE / "data/materiality_mapping_v19.json"
OUT_TSV = HERE / "data/rules_classified_v19_materiality.tsv"
...
def classify_batch(client: OpenAI, system_prompt: str, batch: list[dict]) -> dict:
    user_msg = (
        "Rate resource materiality for each input rule. Return a JSON list.\n"
    ...
    for item in results:
        i = item.get("i")
        m = item.get("resource_materiality")
        reason = item.get("materiality_reasoning", "")
        try:
            m = int(m) if m is not None else None
        except Exception:
            m = None
        out[i] = {"resource_materiality": m, "materiality_reasoning": reason}
    return out


def main():
    system_prompt = PROMPT_FILE.read_text()
    rules = pd.read_csv(SECONDARY_TSV, sep="\t")
    ...
    if todo_rows:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(futs), desc="Materiality"):
                out = fut.result()
                if "error" in out:
    ...
    rules["resource_materiality"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("resource_materiality")
    )
    rules["materiality_reasoning"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("materiality_reasoning", "")
    )

    rules.to_csv(OUT_TSV, sep="\t", index=False)
    print(f"\nWrote {len(rules):,} rules → {OUT_TSV}")

    print("\nResource materiality distribution:")
    print(
        rules["resource_materiality"].value_counts(dropna=False).sort_index().to_string()
    )
