"""
Resource-generality classifier for V19 rules.

For every rule in data/rules_classified_v19_full.tsv, call Gemini with
prompt_V19_resource_generality.md and obtain:
- resource_generality (int 1-5)
- resource_generality_reasoning (short string)

Outputs:
- data/resource_generality_mapping_v19.json    cache keyed by rule_uid
...
PROMPT_FILE = HERE / "prompt/prompt_V19_resource_generality.md"
MAP_JSON = HERE / "data/resource_generality_mapping_v19.json"
...
def classify_batch(client: OpenAI, system_prompt: str, batch: list[dict]) -> dict:
    user_msg = (
        "Rate resource generality for each input rule. Return a JSON list.\n"
    ...
    for item in results:
        i = item.get("i")
        m = item.get("resource_generality")
        reason = item.get("resource_generality_reasoning", "")
        try:
            m = int(m) if m is not None else None
        except Exception:
            m = None
        out[i] = {"resource_generality": m, "resource_generality_reasoning": reason}
    return out
...
def main():
    ...
    if todo_rows:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(futs), desc="Resource gen"):
                out = fut.result()
                if "error" in out:
    ...
    rules["resource_generality"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("resource_generality")
    )
    rules["resource_generality_reasoning"] = rules["rule_uid"].map(
        lambda u: (cache.get(u) or {}).get("resource_generality_reasoning", "")
    )

    rules.to_csv(OUT_TSV, sep="\t", index=False)
    print(f"\nWrote {len(rules):,} rules → {OUT_TSV}")

    print("\nResource generality distribution:")
    print(
        rules["resource_generality"].value_counts(dropna=False).sort_index().to_string()
    )
