...
SAMPLE_TSV = HERE / "data/sample60_v19_batch2.tsv"
...
def main():
    ...
    exclude_uids = set(batch1["rule_uid"])
    before = len(rules)
    rules = rules[~rules["rule_uid"].isin(exclude_uids)].copy()
    print(f"After excluding batch-1 rule_uids ({len(exclude_uids)}): {len(rules):,} (was {before:,})")
    ...
    out = pd.DataFrame({
        ...
        "extraction_method": "gemini_v19_batch2",
        "extraction_cost_usd": "",
        "prompt_tokens": "",
        "completion_tokens": "",
    ...
    print(f"Explorer CSV (batch 2): {EXPLORER_CSV}")
