"""Build a second 60-rule annotation sample — filtered + excluding round 1.

Applies the data-driven filter `resource_materiality >= 3 AND
resource_generality >= 3 AND group_immutability >= 2` (which hit 82.6 %
accuracy on the round-1 labels) to the full 426-rule pool, then excludes
the 60 rule_uids already annotated in round 1, and picks 60 new rules
stratified across periods with (file_id, first-criterion) diversity.

Reads:
  - data/llm_results/core_v1_dimensions/rules_scored.tsv
  - data/annotation/user_comments_sample60_v20.csv      (round-1 labels)
  - data/processed_data/final_dataset_for_criteria.tsv   (for `year`)

Writes:
  - exploring interface/explorer-app/public/data/sample60_v20_round2.csv
  - data/annotation/sample60_v20_round2_raw.tsv
"""
...
ROUND1 = REPO / "data/annotation/user_comments_sample60_v20.csv"
META = REPO / "data/processed_data/final_dataset_for_criteria.tsv"
APP_CSV = REPO / "exploring interface/explorer-app/public/data/sample60_v20_round2.csv"
RAW_TSV = REPO / "data/annotation/sample60_v20_round2_raw.tsv"
...
def row_to_app_csv(row: pd.Series) -> dict:
    ...
    return {
        ...
        "sampled_for": "gemini_v20_sample60_round2",
...
def main() -> None:
    ...
    # Exclude round-1 already-annotated rules.
    r1 = pd.read_csv(ROUND1)
    r1 = r1[r1["rule_uid"].notna() & r1["rule_uid"].astype(str).str.startswith("tlg")]
    done = set(r1["rule_uid"])
    print(f"Excluding {len(done)} round-1 rule_uids")

    # Apply the data-driven filter.
    mask = (
        (df["resource_materiality"] >= 3)
        & (df["resource_generality"] >= 3)
        & (df["group_immutability"] >= 2)
    )
    pool = df[mask & ~df["rule_uid"].isin(done)].copy()
    print(f"Pool: {len(pool)} rules (filter-passing AND not in round 1)")
    print("\nPool per period:")
    print(pool["period"].value_counts().to_string())

    quotas = allocate_quotas(pool, TARGET_PER_PERIOD, TARGET_TOTAL)
    print("\nQuotas (target 15/period with shortfall redistribution):")
    ...
    assert not (set(sample["rule_uid"]) & done), "overlap with round 1!"
