...
BATCH3_TSV = HERE / "data/sample60_v19_batch3.tsv"
SAMPLE_TSV = HERE / "data/sample60_v19_batch4.tsv"
...
def main():
    ...
    # Quality-dimension filters for the new batch (target: 90.9% acc / 470 rules)
    before = len(rules)
    rules = rules[rules["group_specificity"] >= 4].copy()
    print(f"After group_generality >= 4: {len(rules):,} (was {before:,})")

    before = len(rules)
    rules = rules[rules["group_immutability"] >= 4].copy()
    print(f"After group_immutability >= 4: {len(rules):,} (was {before:,})")

    before = len(rules)
    rules = rules[rules["resource_materiality"] >= 2].copy()
    print(f"After resource_materiality >= 2: {len(rules):,} (was {before:,})")

    before = len(rules)
    rules = rules[rules["resource_persistence"] >= 3].copy()
    print(f"After resource_persistence >= 3: {len(rules):,} (was {before:,})")

    before = len(rules)
    rules = rules[rules["tautological"] == 0].copy()
    print(f"After tautological == 0: {len(rules):,} (was {before:,})")

    batch1 = pd.read_csv(BATCH1_TSV, sep="\t")
    batch2 = pd.read_csv(BATCH2_TSV, sep="\t")
    batch3 = pd.read_csv(BATCH3_TSV, sep="\t")
    exclude_uids = set(batch1["rule_uid"]) | set(batch2["rule_uid"]) | set(batch3["rule_uid"])
    before = len(rules)
    rules = rules[~rules["rule_uid"].isin(exclude_uids)].copy()
    print(f"After excluding batches 1-3 rule_uids ({len(exclude_uids)}): {len(rules):,} (was {before:,})")
    ...
    out = pd.DataFrame({
        ...
        "extraction_method": "gemini_v19_batch4",
        "extraction_cost_usd": "",
        "prompt_tokens": "",
        "completion_tokens": "",
    ...
    print(f"Explorer CSV (batch 4): {EXPLORER_CSV}")
